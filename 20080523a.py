...
class Objective:
    ...
    def __init__(self, tree, rate_matrix):
        ...
        @param rate_matrix: a rate matrix used to calculate the likelihood of a column
        """
        self.tree = tree
    ...
    def __call__(self, rate):
        ...
        @return: the negative likelihood of the column using the tree, matrix, and rate
        """
        if not rate:
            inf = float('inf')
            neginf = float('-inf')
...
def get_mle_rates(tree, alignment, rate_matrix):
    ...
    for column in alignment.columns:
        ...
        if column_tuple in column_to_mle_rate:
            ...
            if len(set(column)) == 1:
                # if the column is homogeneous then we know that the mle rate is zero
                mle_rate = 0
            else:
                ...
                # Get the maximum likelihood rate for the column using a golden section search.
                # The bracket is just a suggestion.
                bracket = (0.51, 2.01)
                result = scipy.optimize.golden(objective_function, brack=bracket)
                mle_rate = result
...
def get_form():
    ...
    form_objects = [
            ...
            Form.MultiLine('alignment', 'nucleotide alignment', default_alignment_string),
            Form.MultiLine('weights', 'nucleotide weights', '\n'.join(default_nt_lines)),
            Form.Float('kappa', 'transition transversion rate ratio', 2, low_inclusive=0)]
    return form_objects
...
def get_response(fs):
    ...
    row_major_rate_matrix = RateMatrix.get_unscaled_hky85_rate_matrix(distribution, fs.kappa).get_row_major_rate_matrix()
    rate_matrix = RateMatrix.FastRateMatrix(row_major_rate_matrix, list('ACGT'))
...
def main():
    ...
    kappa = 2.0
    row_major_rate_matrix = RateMatrix.get_unscaled_hky85_rate_matrix(distribution, kappa).get_row_major_rate_matrix()
    rate_matrix = RateMatrix.FastRateMatrix(row_major_rate_matrix, list('ACGT'))
    rate_matrix.normalize()
    # get the mle_rates
