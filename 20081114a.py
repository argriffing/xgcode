...
def get_form():
    ...
    form_objects = [Form.MultiLine('trees', 'newick trees (one tree per line)', '\n'.join(default_tree_strings))]
    return form_objects
...
def get_vector(distance_matrix):
    ...
    """
    D = numpy.array(distance_matrix)
    n = len(D)
    H = numpy.eye(n,n) - numpy.ones((n,n))/n
    HDH = numpy.dot(H, numpy.dot(D, H))
    eigenvalues, eigenvector_transposes = linalg.eigh(HDH)
    eigenvectors = eigenvector_transposes.T
    max_w, max_v = max([(abs(w), v) for w, v in zip(eigenvalues, eigenvectors)])
    return max_v
...
def get_response(fs):
    ...
    for tree in trees:
        ...
        D_full = tree.get_partial_distance_matrix([id(node) for node in all_nodes])
        y_full = get_vector(D_full).tolist()
        y = y_full[:len(tip_nodes)]
        name_selection = frozenset(node.get_name() for node, elem in zip(tip_nodes, y) if elem > 0)
        ...
        if name_partition_a not in valid_partitions:
            print >> local_out, 'augmented distance matrix split fail:', name_partition_a
            has_event = True
        ...
        if name_partition_b not in valid_partitions:
            print >> local_out, 'not-augmented distance matrix split fail:', name_partition_b
            has_event = True
        ...
        if name_partition_a == name_partition_b:
            ...
            diff_count += 1
            print >> local_out, 'this tree was split differently by the different methods:'
            print >> local_out, 'augmented distance matrix split:', name_partition_a
            print >> local_out, 'not-augmented distance matrix split:', name_partition_b
            has_event = True
        # print a newline between trees
        if has_event:
            print >> out, local_out.getvalue()
    # write the summary
    print >> out, 'for this many trees the same split was found:', same_count
    print >> out, 'for this many trees different splits were found:', diff_count
    # write the response
...
def main():
    ...
    print 'correspond to a cut of a single branch on the tree from which the graph was derived?'
    print 'To look for a counterexample to this conjecture, I am sampling a bunch of random trees.'
    print 'Force the program to stop running by using control c when you get bored.'
    try:
        count = 0
        ncounterexamples = 0
        while True:
            ...
            valid_partitions = TreeComparison.get_partitions(tree)
            # get the partition implied by the Fiedler split of the graph derived from the tree
        ...
        print 'found', ncounterexamples, 'counterexamples out of', count, 'attempts'
