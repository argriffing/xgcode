"""
This copypasted script assumes that population size has changed.

The empirical codon stationary distribution is assumed to be a "fossil"
which does not accurately represent the equilibrium of the rate matrix.
However, it is assumed to represent a combination of the current
mutational process stationary distribution with rescaled codon preference
selection parameters, where the scaling is an unknown parameter which
is to be maximized in a numerical log likelihood optimization search.
"""
...
def inner_eval_f(
    ...
    pre_Q_prefix = get_Q_prefix(
            ts, tv, syn, nonsyn,
            log_mu, log_kappa, log_omega)
    return -get_conditional_log_likelihood(
            pre_Q_prefix, pre_Q_suffix, subs_counts)
...
def eval_f_unconstrained(
        theta,
        mu_empirical, subs_counts, log_counts,
    ...
    if boxed_guess[0] is None:
        ...
        v_empirical = codon_counts / float(numpy.sum(codon_counts))
        pre_Q_prefix = get_Q_prefix(
                ts, tv, syn, nonsyn,
                log_mu, log_kappa, log_omega)
        Q = get_Q(pre_Q_prefix, pre_Q_suffix)
        mu_implied = -numpy.dot(numpy.diag(Q), v_empirical)
