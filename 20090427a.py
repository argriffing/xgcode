...
Each column of the contrast matrix corresponds to a contrast, and each row corresponds to a taxon.
The order of the columns does not matter.
The sign of each column does not matter; multiplying one or more columns by -1 does not affect the tree.
...
def get_form():
    ...
    """
    tree = NewickIO.parse(Contrasts.g_felsenstein_tree_string, FelTree.NewickTree)
    ordered_labels = ('a', 'b', 'c', 'd', 'e')
    C = Contrasts.get_contrast_matrix(tree, ordered_labels)
    # define the form objects
    form_objects = [
            Form.Matrix('contrast_matrix', 'contrast matrix', C, Contrasts.assert_contrast_matrix),
