...
CherryUtil.add_to_path(CherryUtil.g_script_directory)
CherryUtil.add_to_path(CherryUtil.g_extension_directory)
...
cherrypy.config.update({
    'tools.staticdir.root': CherryUtil.g_epydoc_directory,
    'server.socket_host': xgconfig.g_host_address,
    'server.socket_port': xgconfig.g_host_port})


def handle_error():
    cherrypy.response.status = 500
    cherrypy.response.body = ['<html><body>oh no it failed</body></html>']

cherrypy.config.update({
    'request.error.response': handle_error})
...
if __name__ == '__main__':
    print 'ohai im starting the webserver now'
